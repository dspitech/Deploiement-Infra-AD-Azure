...
function New-GpoIfMissing {
    ...
    if (-not $gpo) {
        $gpo = New-GPO -Name $Name -Comment "Genere par script PowerShell - ESTIAM"
...
$serverName = $env:COMPUTERNAME   # nom reel du serveur de fichiers (SRV-AD01 par defaut)
$logonScriptContent = @"
@echo off
:: Script de connexion ESTIAM - monte les lecteurs reseau selon le service
net use H: \\$serverName\Users$\%USERNAME% /persistent:no >nul 2>&1
net use P: \\$serverName\Public /persistent:no >nul 2>&1

net user %USERNAME% /domain 1>nul 2>nul
whoami /groups | findstr /I "GG-ESTIAM-FINANCE" >nul && net use F: \\$serverName\Finance /persistent:no >nul 2>&1
whoami /groups | findstr /I "GG-ESTIAM-RH" >nul && net use R: \\$serverName\RH /persistent:no >nul 2>&1
whoami /groups | findstr /I "GG-ESTIAM-DIRECTION" >nul && net use O: \\$serverName\Direction /persistent:no >nul 2>&1
whoami /groups | findstr /I "GG-ESTIAM-IT" >nul && net use I: \\$serverName\IT /persistent:no >nul 2>&1
whoami /groups | findstr /I "GG-ESTIAM-MARKETING" >nul && net use M: \\$serverName\Marketing /persistent:no >nul 2>&1
whoami /groups | findstr /I "GG-ESTIAM-ADMINISTRATION" >nul && net use A: \\$serverName\Administration /persistent:no >nul 2>&1
"@
