<#
.SYNOPSIS
    Durcissement complementaire du poste client PC-CLIENT01 (GPO immediates,
    AppLocker, Defender/pare-feu, BitLocker avec sauvegarde de la cle dans AD).

.DESCRIPTION
    A executer sur PC-CLIENT01, en administrateur, APRES :
      - 01-join-domain.ps1 (poste joint au domaine et redemarre),
      - l'execution des etapes serveur (au minimum 10-gpo).
    Se connecter avec un compte administrateur du domaine (ex : ESTIAM\estiamadmin).
    Vient en complement des GPO et force leur application immediate.
#>
